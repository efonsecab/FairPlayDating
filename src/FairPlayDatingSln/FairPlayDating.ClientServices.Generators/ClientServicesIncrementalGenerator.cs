...
namespace FairPlayDating.ClientServices.Generators
{
    [Generator]
                ...
                foreach (var singleAttributeList in singleClassDeclarationSyntax.AttributeLists)
                {
                    foreach (var singleAttribute in singleAttributeList.Attributes)
                    {
                        ...
                        if (identifierText == "ClientServiceOfEntity")
                        {
                            ...
                            stringBuilder.AppendLine("var response = await authorizedHttpClient.PostAsJsonAsync(requestUrl,model);");
                            ...
                            stringBuilder.AppendLine($"var result = await authorizedHttpClient.GetFromJsonAsync<{entityName}Model[]>(requestUrl);");
                            stringBuilder.AppendLine("return result;");
                            stringBuilder.AppendLine("}");
                            ...
                            stringBuilder.AppendLine("var authorizedHttpClient = _httpClientService.CreateAuthorizedClient();");
                            stringBuilder.AppendLine("var response = await authorizedHttpClient.DeleteAsync(requestUrl);");
                            stringBuilder.AppendLine("response.EnsureSuccessStatusCode();");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
                            stringBuilder.AppendLine("}");
